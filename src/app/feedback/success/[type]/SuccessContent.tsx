...
import { ArrowRight, LucideIcon, ThumbsUp } from 'lucide-react'
...
export default function SuccessContent({ config }: SuccessContentProps) {
  const { icon, iconColor, title, description, action } = config
  const Icon = ICON_MAP[icon] || ThumbsUp
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        <motion.div
          className="glass rounded-3xl p-12 border border-white/10 backdrop-blur-sm shadow-2xl bg-gradient-to-br from-background/80 via-background/60 to-green-500/5"
          initial={{ scale: 0.95 }}
          animate={{ scale: 1 }}
        ...
        >
          ...
          >
            <div className="mx-auto w-24 h-24 bg-green-500/10 rounded-full flex items-center justify-center mb-6 shadow-lg">
